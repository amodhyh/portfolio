...
export function OperationsLedger({ operations, academics }: { operations: Operation[], academics: Academic[] }) {
  return (
    <section className="mb-20 ">
      ...
      {/* Academics */}
      <div>
        <h2 className="uppercase tracking-widest text-slate-500 font-semibold mb-6">Academic Baseline</h2>
        <div className="grid gap-8">
          {academics.map((academic, idx) => (
            <div key={idx} className="border-l-2 border-slate-300 pl-5 py-1">
              <div className="flex justify-between items-baseline mb-1">
                <h3 className="text-base font-bold text-slate-900">{academic.degree}</h3>
              </div>
              <div className="flex justify-between items-baseline mb-4">
                <p className="text-slate-700 font-medium text-sm">{academic.institution}</p>
                <span className="text-slate-500 uppercase tracking-widest text-[10px]">{academic.period}</span>
              </div>
              <ul className="space-y-2 text-slate-600 list-none text-sm">
